...
for i in 1:3
    for gen in generaciones
        ini = time_ns()
        p, califFinal, genomaFinal = algoritmoGeneticoReporte(calif, tipos[i], pobs[i], genes; intStart=1, intEnd=size(mtz,1 ), random=0, Pm=pms[i])
        println(tipo, " Tiempo:"(time_ns() - ini )/ 1e9, ", calif:" califFinal)
    end
